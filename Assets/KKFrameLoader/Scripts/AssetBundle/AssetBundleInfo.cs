...
namespace KK.Frame.Loader.ABS
{
    public class AssetBundleInfo
    {
        ...
        {
            Debug.Log("<color=green>[log]</color>---" + "Retain:" + bundleName + "-" + references.Count);
            if (owner == null)
        ...
        {
            Debug.Log("<color=green>[log]</color>---" + "Release:" + bundleName + "-" + references.Count);
            for (int i = 0; i < references.Count; i++)
            {
        ...
        public virtual GameObject Instantiate(bool enable)
        {
            if (mainObject != null)
            {
                ...
                if (mainObject is GameObject)
                {
                    ...
                    Object inst = ObjectPoolController.Instantiate(prefab);
        ...
        public virtual GameObject Instantiate(Vector3 position, Quaternion rotation, bool enable = true)
        {
            if (mainObject != null)
            {
                ...
                if (mainObject is GameObject)
                {
                    GameObject prefab = mainObject as GameObject;
                    prefab.SetActive(enable);
                    Object inst = ObjectPoolController.Instantiate(prefab, position, rotation);
                    inst.name = prefab.name;
                    Retain(inst);
                    return (GameObject)inst;
                }
        ...
        /// <param name="user">增加引用的对象</param>
        /// <returns></returns>
        public virtual Object Require(Object user)
        {
            this.Retain(user);
            return mainObject;
        }

        public virtual T Require<T>(Object user) where T : UnityEngine.Object
        {
#if AB_MODE
            this.Retain(user);
            string[] names = bundle.GetAllAssetNames();
            return bundle.LoadAsset<T>(names[0]);
#else
        ...
        /// <returns></returns>
        public virtual Object Require(Component c, bool autoBindGameObject)
        {
        ...
        int UpdateReference()
        {
            for (int i = 0; i < references.Count; i++)
            {
                ...
                if (!o)
                {
                    Debug.Log("<color=green>[log]</color>---" + "removeat:" + bundleName);
                    references.RemoveAt(i);
                    i--;
                }
            }
        ...
        internal virtual UnityEngine.Object[] LoadAllAssets(Object user)
        {
            this.Retain(user);
